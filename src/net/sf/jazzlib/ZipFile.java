...
public class ZipFile implements ZipConstants
{
   ...
   * archive.  
   */
  public ZipFile(String path) throws ZipException, IOException
  {
    this.raf = new RandomAccessFile(path);
    this.name = path;
  }
  /**
   ...
   */
  private final int readLeShort(RandomAccessFile di, byte[] b) throws IOException
  {
    di.readFully(b, 0, 2);
   ...
   * @exception EOFException if the file ends prematurely
   */
  private final int readLeInt(RandomAccessFile di, byte[] b) throws IOException
  {
    di.readFully(b, 0, 4);
    return ((b[0] & 0xff) | (b[1] & 0xff) << 8)
